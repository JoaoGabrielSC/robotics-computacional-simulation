...
class KinematicController:
    ...
    def compute_control(self, current_pos: np.ndarray, t: float) -> np.ndarray:
        ...
        if 0 < distance_to_obs < self.config.rho_0:
            ...
            magnitude = self.config.k_rep * (1.0 / distance_to_obs - 1.0 / self.config.rho_0) * (1.0 / (distance_to_obs ** 2))
...
def run_simulation():
    ...
    animate_results(time_steps, np.array(robot.history), x_d_hist, y_d_hist, obs_pos, config.dt)
...
def animate_results(time_steps, history, x_d_hist, y_d_hist, obs_pos, dt):
    ...
    frames = range(0, len(time_steps), step)
